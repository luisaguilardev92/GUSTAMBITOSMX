...
const withLootHacker = (key: string, variants: Variant[], available = true): Variant[] => [...variants, { label: "Loot Hacker", image: spriteUrl(key, "Hacker"), level: 0, available }];
const announcedVariants = (): Variant[] => ["Base", "Dorado", "Cheat Master", "Loot Hacker"].map((label) => ({ label, image: "/sprite-coming-soon.svg", level: 0, available: false }));
const initialGustambitos: Gustambito[] = [
  ...
  { id: 3, name: "Arbustín", subtitle: "Se camufla en la isla", rarity: "Raro", color: "#79c85b", image: spriteUrl("BushRanger"), season: "GLITCH · Capítulo 7", variants: withLootHacker("BushRanger", makeVariants("BushRanger")) },
  ...
  { id: 17, name: "Crash Bandicoot", subtitle: "Un salto desata un torbellino", rarity: "Épico", color: "#f18434", image: "/sprite-coming-soon.svg", season: "LLEGA · 17 SEP", variants: announcedVariants() },
  { id: 18, name: "Blinky", subtitle: "Se camufla al recibir daño", rarity: "Épico", color: "#ed4054", image: "/sprite-coming-soon.svg", season: "LLEGA · 17 SEP", variants: announcedVariants() },
  { id: 19, name: "Morgana", subtitle: "Mejora la eficacia de curación", rarity: "Épico", color: "#e8e8ef", image: "/sprite-coming-soon.svg", season: "LLEGA · 17 SEP", variants: announcedVariants() },
  { id: 20, name: "Pond", subtitle: "Salto súper cargado", rarity: "Épico", color: "#54c58b", image: "/sprite-coming-soon.svg", season: "LLEGA · 17 SEP", variants: announcedVariants() },
  { id: 21, name: "Birthday", subtitle: "Pastel sorpresa en cofres", rarity: "Épico", color: "#f18cc3", image: "/sprite-coming-soon.svg", season: "LLEGA · 17 SEP", variants: announcedVariants() },
];
...
async function downloadCollectionImage(items: Gustambito[], friendCode: string) {
  ...
  const groupHeight = 272;
  const cardWidth = 132;
  const cardHeight = 204;
  const cardsPerGroup = 4;
  const canvas = document.createElement("canvas");
  canvas.width = columns * groupWidth;
  canvas.height = 190 + Math.ceil(items.length / columns) * groupHeight + 86;
  ...
  for (const [index, item] of items.entries()) {
    const groupX = (index % columns) * groupWidth + 10;
    const groupY = 150 + Math.floor(index / columns) * groupHeight;
    context.fillStyle = "rgba(8,18,53,.92)";
    context.fillRect(groupX, groupY, groupWidth - 20, groupHeight - 16);
    context.strokeStyle = "#39e8ff";
    context.strokeRect(groupX, groupY, groupWidth - 20, groupHeight - 16);
    context.strokeStyle = "rgba(255,54,186,.8)";
    context.strokeRect(groupX + 5, groupY - 5, groupWidth - 20, groupHeight - 16);
    context.fillStyle = "#ffd84d";
    context.font = "900 16px Arial";
    context.fillText(item.name.toUpperCase(), groupX + 12, groupY + 25);
    for (const [variantIndex, variant] of item.variants.slice(0, cardsPerGroup).entries()) {
      const x = groupX + 10 + variantIndex * (cardWidth + 5);
      const y = groupY + 42;
      ...
      if (variant.level > 0) {
        ...
        context.drawImage(sprite, x + 18, y + 6, 96, 105);
        context.globalAlpha = 1;
        context.filter = "none";
      }
      context.drawImage(sprite, x + 20, y + 6, 92, 105);
      context.restore();
      if (variant.level === 5) context.drawImage(crown, x + 75, y - 2, 38, 29);
      context.fillStyle = variant.level > 0 ? "#fff" : "#6d7e9b";
      context.font = "900 10px Arial";
      context.fillText(variant.label.toUpperCase(), x + 8, y + 128);
      context.fillStyle = variant.level > 0 ? "#39e8ff" : "#687a95";
      context.font = "900 10px Arial";
      context.fillText(`NIVEL ${variant.level}/5`, x + 8, y + 149);
    }
  }
