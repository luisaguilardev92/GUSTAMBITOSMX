...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const markers = [
      { id: "drift-legs", x: 26.5, y: 45.9, name: "DRIFT · PIERNAS", kind: "wrixel", detail: "Colina central de Green Hill" }, { id: "drift-accessory", x: 28.5, y: 44.5, name: "DRIFT · ACCESORIO", kind: "wrixel", detail: "Junto a la cascada" }, { id: "drift-hat", x: 30, y: 47.6, name: "DRIFT · SOMBRERO", kind: "wrixel", detail: "Cruce de caminos verdes" }, { id: "drift-torso", x: 32, y: 48.2, name: "DRIFT · TORSO", kind: "wrixel", detail: "Cliff bajo el sitio de extracción" },
      { id: "luxe-accessory", x: 74.1, y: 88, name: "LUXE · ACCESORIO", kind: "wrixel", detail: "Cubierta del yate pato" }, { id: "luxe-torso", x: 77, y: 89.7, name: "LUXE · TORSO", kind: "wrixel", detail: "Sauna junto a la piscina" }, { id: "luxe-legs", x: 79.6, y: 89.3, name: "LUXE · PIERNAS", kind: "wrixel", detail: "Piscina interior" }, { id: "luxe-hat", x: 77.6, y: 90.7, name: "LUXE · SOMBRERO", kind: "wrixel", detail: "Detrás del hotel" },
      { id: "skull-ranger-accessory", x: 53, y: 75.4, name: "SKULL RANGER · ACCESORIO", kind: "wrixel", detail: "Esquina noreste del laberinto" }, { id: "skull-ranger-hat", x: 54.2, y: 76, name: "SKULL RANGER · SOMBRERO", kind: "wrixel", detail: "Centro del laberinto" }, { id: "skull-ranger-torso", x: 52.8, y: 77.7, name: "SKULL RANGER · TORSO", kind: "wrixel", detail: "Corredor oeste inferior" }, { id: "skull-ranger-legs", x: 55, y: 77, name: "SKULL RANGER · PIERNAS", kind: "wrixel", detail: "Corredor sur" },
      { id: "ruby-torso", x: 42.1, y: 30.9, name: "RUBY · TORSO", kind: "wrixel", detail: "Calle sur junto al edificio azul" }, { id: "ruby-legs", x: 43.4, y: 33.1, name: "RUBY · PIERNAS", kind: "wrixel", detail: "Parque junto al árbol rojo" }, { id: "ruby-hat", x: 38.6, y: 37.1, name: "RUBY · SOMBRERO", kind: "wrixel", detail: "Edificio de puertas amarillas" }, { id: "ruby-accessory", x: 41.6, y: 36.1, name: "RUBY · ACCESORIO", kind: "wrixel", detail: "Canal bajo el puente" },
      { id: "cheat-green-hill", x: 25, y: 50, name: "GREEN HILL ZONE", kind: "cheat", detail: "Zona Cheat Code" }, { id: "cheat-cluster-coast", x: 77, y: 91, name: "CLUSTER COAST", kind: "cheat", detail: "Zona de alta densidad" }, { id: "cheat-reality-reign", x: 60, y: 50, name: "REALITY'S REIGN", kind: "cheat", detail: "Zona de alta densidad" }, { id: "cheat-heatwave", x: 13, y: 75, name: "HEATWAVE HARBOR", kind: "cheat", detail: "Zona de alta densidad" }, { id: "cheat-shaken", x: 68, y: 74, name: "SHAKEN SANCTUARY", kind: "cheat", detail: "Zona Cheat Code" },
    ];
    const markerHtml = markers.map((marker, index) => { const markerNumber = marker.kind === "wrixel" ? (index % 4) + 1 : ""; return `<button class="map-marker ${marker.kind}" data-map-kind="${marker.kind}" data-marker-id="${marker.id}" data-part-id="${marker.kind === "wrixel" ? marker.id : ""}" title="${marker.name} · ${marker.detail}" aria-label="${marker.name}" style="left:${marker.x}%;top:${marker.y}%"><span class="map-marker-dot">${markerNumber}</span></button>`; }).join("");
    overlay.innerHTML = `<div class="map-panel"><button class="map-close" aria-label="Cerrar">×</button><div class="map-heading"><div><p class="eyebrow">GLITCH · ISLA OVERRIDE</p><h2>MAPA</h2><p>Toca un punto amarillo cuando consigas una pieza de Wrixel.</p></div><div class="map-controls"><button data-map-filter="all" class="active">TODOS</button><button data-map-filter="wrixel">WRIXEL</button><button data-map-filter="cheat">CHEAT CODES</button><button data-map-calibrate>AJUSTAR PUNTOS</button></div></div><div class="map-viewport"><div class="map-canvas"><img src="https://fortnite-api.com/images/map.png" alt="Mapa de la isla Fortnite Override" /><div class="map-markers">${markerHtml}</div></div></div><div class="map-calibration"><button data-map-copy-log>COPIAR LOG</button><textarea data-map-log readonly aria-label="Log de coordenadas"></textarea></div><div class="map-legend"><span class="map-key"><i class="map-key-dot pending"></i> WRIXEL PENDIENTE</span><span class="map-key"><i class="map-key-dot collected"></i> WRIXEL CONSEGUIDO</span><span class="map-key"><i class="map-key-dot cheat"></i> CHEAT CODE</span><small data-map-status>Toca los puntos Wrixel para guardarlos en tu cuenta</small></div></div>`;
    document.body.append(overlay);
    ...
    const calibrateButton = overlay.querySelector<HTMLButtonElement>("[data-map-calibrate]");
    const calibrationBox = overlay.querySelector<HTMLElement>(".map-calibration");
    const calibrationLog = overlay.querySelector<HTMLTextAreaElement>("[data-map-log]");
    let calibrating = false;
    let draggingMarker: HTMLElement | null = null;
    const writeCalibrationLog = () => { if (!calibrationLog) return; const entries = [...overlay.querySelectorAll<HTMLElement>(".map-marker")].map((marker) => ({ id: marker.dataset.markerId, x: Number.parseFloat(marker.style.left), y: Number.parseFloat(marker.style.top) })); calibrationLog.value = JSON.stringify(entries, null, 2); };
    const onMarkerPointerMove = (event: PointerEvent) => { if (!draggingMarker || !canvas) return; const rect = canvas.getBoundingClientRect(); const x = Math.max(0, Math.min(100, ((event.clientX - rect.left) / rect.width) * 100)); const y = Math.max(0, Math.min(100, ((event.clientY - rect.top) / rect.height) * 100)); draggingMarker.style.left = `${x}%`; draggingMarker.style.top = `${y}%`; writeCalibrationLog(); event.preventDefault(); };
    const onMarkerPointerUp = () => { draggingMarker = null; };
    calibrateButton?.addEventListener("click", () => { calibrating = !calibrating; overlay.classList.toggle("calibration-mode", calibrating); calibrateButton.textContent = calibrating ? "SALIR DE AJUSTE" : "AJUSTAR PUNTOS"; if (calibrating) { if (calibrationBox) calibrationBox.hidden = false; writeCalibrationLog(); if (mapStatus) mapStatus.textContent = "Arrastra los puntos y copia el log cuando termines"; } });
    overlay.querySelector<HTMLButtonElement>("[data-map-copy-log]")?.addEventListener("click", async () => { if (!calibrationLog) return; await navigator.clipboard?.writeText(calibrationLog.value); if (mapStatus) mapStatus.textContent = "Log copiado"; });
    overlay.querySelectorAll<HTMLElement>(".map-marker").forEach((marker) => marker.addEventListener("pointerdown", (event) => { if (!calibrating) return; draggingMarker = marker; marker.setPointerCapture?.(event.pointerId); event.preventDefault(); }));
    ...
    overlay.querySelectorAll<HTMLElement>(".map-marker.wrixel").forEach((marker) => marker.addEventListener("click", async () => { if (calibrating) return; const partKey = marker.dataset.partId || ""; const next = !marker.classList.contains("collected"); setCollected(marker, next); const response = await fetch("/api/wrixel-parts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ partKey, collected: next }) }); if (!response.ok) { setCollected(marker, !next); if (mapStatus) mapStatus.textContent = "No se pudo guardar la pieza"; } else if (mapStatus) mapStatus.textContent = next ? "Pieza guardada en tu cuenta" : "Pieza marcada como pendiente"; }));
    ...
    overlay.addEventListener("click", (event) => { if (event.target === overlay) hide(); });
    actions.append(button);
    return () => { viewport?.removeEventListener("touchstart", onTouchStart); viewport?.removeEventListener("touchmove", onTouchMove); viewport?.removeEventListener("touchend", onTouchEnd); overlay.removeEventListener("pointermove", onMarkerPointerMove); overlay.removeEventListener("pointerup", onMarkerPointerUp); button.remove(); overlay.remove(); };
  }, [status]);
