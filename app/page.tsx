...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const markerHtml = markers.map((marker, index) => { const markerNumber = marker.kind === "wrixel" ? (index % 4) + 1 : ""; const mapX = 5 + marker.x * .85; const mapY = 7 + marker.y * .86; return `<button class="map-marker ${marker.kind}" data-map-kind="${marker.kind}" data-part-id="${marker.kind === "wrixel" ? marker.id : ""}" title="${marker.name} · ${marker.detail}" aria-label="${marker.name}" style="left:${mapX}%;top:${mapY}%"><span class="map-marker-dot">${markerNumber}</span></button>`; }).join("");
    overlay.innerHTML = `<div class="map-panel"><button class="map-close" aria-label="Cerrar">×</button><div class="map-heading"><div><p class="eyebrow">GLITCH · ISLA OVERRIDE</p><h2>MAPA</h2><p>Toca un punto amarillo cuando consigas una pieza de Wrixel.</p></div><div class="map-controls"><button data-map-filter="all" class="active">TODOS</button><button data-map-filter="wrixel">WRIXEL</button><button data-map-filter="cheat">CHEAT CODES</button></div></div><div class="map-viewport"><div class="map-canvas"><img src="https://fortnite-api.com/images/map.png" alt="Mapa de la isla Fortnite Override" /><div class="map-markers">${markerHtml}</div></div></div><div class="map-legend"><span class="map-key"><i class="map-key-dot pending"></i> WRIXEL PENDIENTE</span><span class="map-key"><i class="map-key-dot collected"></i> WRIXEL CONSEGUIDO</span><span class="map-key"><i class="map-key-dot cheat"></i> CHEAT CODE</span><small data-map-status>Toca los puntos Wrixel para guardarlos en tu cuenta</small></div></div>`;
    document.body.append(overlay);
