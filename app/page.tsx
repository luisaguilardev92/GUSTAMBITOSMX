...
export default function Home() {
  ...
  useEffect(() => { const sync = async () => { const response = await fetch("/api/progress"); if (response.ok) { const { progress } = await response.json(); if (progress.length) setGustambitos((current) => current.map((item) => ({ ...item, variants: item.variants.map((variant) => { const saved = progress.find((entry: { gustambito_id: number; variant_label: string; level: number }) => entry.gustambito_id === item.id && entry.variant_label === variant.label); return saved ? { ...variant, level: saved.level } : variant; }) }))); } else { const saved = window.localStorage.getItem("gustambitos-glitch-v4") ?? window.localStorage.getItem("gustambitos-glitch-v3"); if (saved) setGustambitos(migrate(JSON.parse(saved))); } window.requestAnimationFrame(() => setHydrated(true)); }; sync(); }, []);
  const visible = hydrated ? gustambitos : initialGustambitos;
  const setLevel = (id: number, variantIndex: number, level: number) => { const item = gustambitos.find((entry) => entry.id === id); const variant = item?.variants[variantIndex]; const next = gustambitos.map((entry) => entry.id !== id ? entry : { ...entry, variants: entry.variants.map((current, index) => index === variantIndex ? { ...current, level } : current) }); setGustambitos(next); window.localStorage.setItem("gustambitos-glitch-v4", JSON.stringify(next)); if (variant) void fetch("/api/progress", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ gustambitoId: id, variantLabel: variant.label, level }) }); };
