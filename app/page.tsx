...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const openGuide = (marker: HTMLElement) => { if (!mapDetail || !mapDetailImage) return; mapDetail.hidden = false; mapDetailImage.src = marker.dataset.guideImage || ""; mapDetailTitle!.textContent = marker.title.split(" · ")[0]; mapDetailZone!.textContent = marker.title.split(" · ")[1] || "UBICACIÓN WRIXEL"; mapDetailCopy!.textContent = "Referencia visual de la caja dentro del juego."; };
    ...
    overlay.querySelectorAll<HTMLElement>(".map-marker.wrixel").forEach((marker) => marker.addEventListener("click", async () => { openGuide(marker); const partKey = marker.dataset.partId || ""; const next = !marker.classList.contains("collected"); setCollected(marker, next); const response = await fetch("/api/wrixel-parts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ partKey, collected: next }) }); if (!response.ok) { setCollected(marker, !next); if (mapStatus) mapStatus.textContent = "No se pudo guardar la pieza"; } else if (mapStatus) mapStatus.textContent = next ? "Pieza guardada en tu cuenta" : "Pieza marcada como pendiente"; }));
