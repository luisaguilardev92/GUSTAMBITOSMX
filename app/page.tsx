...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const markerHtml = markers.map((marker, index) => { const markerNumber = marker.kind === "wrixel" ? (markerNumbers[marker.id] || (index % 4) + 1) : ""; const secondImage = marker.id === "drift-torso" ? "https://i0.wp.com/www.gamerfocus.co/wp-content/uploads/2026/08/Fortnite_20260820104014.jpg?resize=1024%2C576&ssl=1" : ""; return `<button class="map-marker ${marker.kind}" data-map-kind="${marker.kind}" data-marker-id="${marker.id}" data-guide-image="${guideImages[index] || ""}" data-guide-image-two="${secondImage}" data-guide-detail="${marker.detail}" data-part-id="${marker.kind === "wrixel" ? marker.id : ""}" title="${marker.name} · ${marker.detail}" aria-label="${marker.name}" style="left:${marker.x}%;top:${marker.y}%"><span class="map-marker-dot">${markerNumber}</span></button>`; }).join("");
