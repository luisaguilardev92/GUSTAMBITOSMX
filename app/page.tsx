...
export default function Home() {
  ...
  useEffect(() => {
    ...
    overlay.innerHTML = `<div class="map-panel"><button class="map-close" aria-label="Cerrar">×</button><div class="map-heading"><div><p class="eyebrow">GLITCH · ISLA OVERRIDE</p><h2>MAPA</h2><p>Toca un punto amarillo cuando consigas una pieza de Wrixel.</p></div><div class="map-controls"><button data-map-filter="all" class="active">TODOS</button><button data-map-filter="wrixel">WRIXEL</button><button data-map-filter="cheat">CHEAT CODES</button></div></div><div class="map-viewport"><div class="map-canvas"><img src="https://fortnite-api.com/images/map.png" alt="Mapa de la isla Fortnite Override" /><div class="map-markers">${markers.map((marker) => `<button class="map-marker ${marker.kind}" data-map-kind="${marker.kind}" data-part-id="${marker.kind === "wrixel" ? marker.id : ""}" title="${marker.name} · ${marker.detail}" aria-label="${marker.name}"><span class="map-marker-dot"></span></button>`).join("")}</div></div></div><div class="map-legend"><span><i class="legend-wrixel"></i> WRIXEL: PENDIENTE</span><span><i class="legend-collected"></i> WRIXEL: CONSEGUIDA</span><span><i class="legend-cheat"></i> ZONA CHEAT CODE</span><small data-map-status>Toca los puntos Wrixel para guardarlos en tu cuenta</small></div></div>`;
    document.body.append(overlay);
