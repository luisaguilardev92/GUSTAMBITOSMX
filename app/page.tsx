...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="wrixel-styles-section" aria-labelledby="wrixel-styles-title"><div className="section-heading"><div><p className="eyebrow">WRIXEL · DESBLOQUEABLES</p><h2 id="wrixel-styles-title">Otros estilos</h2></div><span className="count-badge">11 ESTILOS</span></div><p className="wrixel-styles-intro">Completa misiones, habla con NPC o activa códigos de hackeo para desbloquear más estilos de Wrixel.</p><div className="wrixel-styles-grid">{wrixelStyles.map(([name, requirement], index) => <article className="wrixel-style-card" key={name}><span className="wrixel-style-index">{String(index + 1).padStart(2, "0")}</span><h3>{name}</h3><p>{requirement}</p></article>)}</div><a className="wrixel-styles-link" href="https://www.gamerfocus.co/juegos/lista-actualizada-codigos-hacks-de-sala-panel-de-administracion-fortnite/" target="_blank" rel="noreferrer">VER CÓDIGOS DE HACK DE SALA ↗</a><small className="wrixel-styles-note">Las ubicaciones de códigos y NPC aparecen en el mapa cuando rastreas la misión correspondiente.</small></section>
