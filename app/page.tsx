...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const markers = [
      { id: "drift-legs", x: 31.0371, y: 48.7988, name: "DRIFT · PIERNAS", kind: "wrixel", detail: "Colina central de Green Hill" }, { id: "drift-accessory", x: 26.8126, y: 47.9234, name: "DRIFT · ACCESORIO", kind: "wrixel", detail: "Junto a la cascada" }, { id: "drift-hat", x: 28.5633, y: 46.5152, name: "DRIFT · SOMBRERO", kind: "wrixel", detail: "Cruce de caminos verdes" }, { id: "drift-torso", x: 28.0304, y: 48.8749, name: "DRIFT · TORSO", kind: "wrixel", detail: "Cliff bajo el sitio de extracción" },
      { id: "luxe-accessory", x: 68.373, y: 83.4705, name: "LUXE · ACCESORIO", kind: "wrixel", detail: "Cubierta del yate pato" }, { id: "luxe-torso", x: 69.8953, y: 82.9757, name: "LUXE · TORSO", kind: "wrixel", detail: "Sauna junto a la piscina" }, { id: "luxe-legs", x: 68.6394, y: 84.46, name: "LUXE · PIERNAS", kind: "wrixel", detail: "Piscina interior" }, { id: "luxe-hat", x: 65.6327, y: 82.519, name: "LUXE · SOMBRERO", kind: "wrixel", detail: "Detrás del hotel" },
      { id: "skull-ranger-accessory", x: 49.2175, y: 72.4963, name: "SKULL RANGER · ACCESORIO", kind: "wrixel", detail: "Esquina noreste del laberinto" }, { id: "skull-ranger-hat", x: 48.2398, y: 72.0147, name: "SKULL RANGER · SOMBRERO", kind: "wrixel", detail: "Centro del laberinto" }, { id: "skull-ranger-torso", x: 48.1637, y: 74.0319, name: "SKULL RANGER · TORSO", kind: "wrixel", detail: "Corredor oeste inferior" }, { id: "skull-ranger-legs", x: 50.1978, y: 71.862, name: "SKULL RANGER · PIERNAS", kind: "wrixel", detail: "Corredor sur" },
      { id: "ruby-torso", x: 39.1837, y: 34.6101, name: "RUBY · TORSO", kind: "wrixel", detail: "Calle sur junto al edificio azul" }, { id: "ruby-legs", x: 39.1837, y: 38.8197, name: "RUBY · PIERNAS", kind: "wrixel", detail: "Parque junto al árbol rojo" }, { id: "ruby-hat", x: 39.9333, y: 36.4554, name: "RUBY · SOMBRERO", kind: "wrixel", detail: "Edificio de puertas amarillas" }, { id: "ruby-accessory", x: 36.8194, y: 39.973, name: "RUBY · ACCESORIO", kind: "wrixel", detail: "Canal bajo el puente" },
    ];
    ...
    const markerNumbers: Record<string, number> = { "drift-torso": 1, "drift-accessory": 2, "drift-hat": 3, "drift-legs": 4 };
    const markerHtml = markers.map((marker, index) => { const markerNumber = marker.kind === "wrixel" ? (markerNumbers[marker.id] || (index % 4) + 1) : ""; const secondImage = marker.id === "drift-torso" ? "https://i0.wp.com/www.gamerfocus.co/wp-content/uploads/2026/08/Fortnite_20260820104014.jpg?resize=1024%2C576&ssl=1" : ""; return `<button class="map-marker ${marker.kind}" data-map-kind="${marker.kind}" data-marker-id="${marker.id}" data-guide-image="${guideImages[index] || ""}" data-guide-image-two="${secondImage}" data-part-id="${marker.kind === "wrixel" ? marker.id : ""}" title="${marker.name} · ${marker.detail}" aria-label="${marker.name}" style="left:${marker.x}%;top:${marker.y}%"><span class="map-marker-dot">${markerNumber}</span></button>`; }).join("");
    overlay.innerHTML = `<div class="map-panel"><button class="map-close" aria-label="Cerrar">×</button><div class="map-heading"><div><p class="eyebrow">GLITCH · ISLA OVERRIDE</p><h2>MAPA</h2><p>Toca un punto amarillo para ver la caja dentro del juego.</p></div><div class="map-controls"><button data-map-filter="all" class="active">TODOS</button><button data-map-filter="wrixel">WRIXEL</button></div></div><div class="map-viewport"><div class="map-canvas"><img src="https://fortnite-api.com/images/map.png" alt="Mapa de la isla Fortnite Override" /><div class="map-markers">${markerHtml}</div></div></div><aside class="map-detail" hidden><button class="map-detail-close" aria-label="Cerrar imagen">×</button><p class="eyebrow" data-map-detail-zone>UBICACIÓN WRIXEL</p><h3 data-map-detail-title></h3><div class="map-detail-images"><img data-map-detail-image alt="Captura de ubicación de Wrixel" /><img data-map-detail-image-two alt="Segunda captura de ubicación de Wrixel" hidden /></div><p data-map-detail-copy></p><button class="map-detail-action" data-map-mark-collected>MARCAR COMO CONSEGUIDO</button></aside><div class="map-legend"><span class="map-key"><i class="map-key-dot pending"></i> WRIXEL PENDIENTE</span><span class="map-key"><i class="map-key-dot collected"></i> WRIXEL CONSEGUIDO</span><small data-map-status>Toca un punto para ver la ubicación de la caja</small></div></div>`;
    document.body.append(overlay);
    ...
    const openGuide = (marker: HTMLElement) => { if (!mapDetail || !mapDetailImage) return; activeMarker = marker; mapDetail.hidden = false; mapDetailImage.src = marker.dataset.guideImage || ""; if (mapDetailImageTwo) { mapDetailImageTwo.hidden = !marker.dataset.guideImageTwo; mapDetailImageTwo.src = marker.dataset.guideImageTwo || ""; } mapDetailTitle!.textContent = marker.title.split(" · ")[0]; mapDetailZone!.textContent = marker.title.split(" · ")[1] || "UBICACIÓN WRIXEL"; mapDetailCopy!.textContent = marker.dataset.guideImageTwo ? "Dos referencias visuales de la caja dentro del juego." : "Referencia visual de la caja dentro del juego."; if (mapDetailAction) mapDetailAction.textContent = marker.classList.contains("collected") ? "MARCADO COMO CONSEGUIDO" : "MARCAR COMO CONSEGUIDO"; };
