...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const openGuide = (marker: HTMLElement) => { if (!mapDetail || !mapDetailImage) return; activeMarker = marker; mapDetail.hidden = false; mapDetailImage.src = marker.dataset.guideImage || ""; mapDetailTitle!.textContent = marker.title.split(" · ")[0]; mapDetailZone!.textContent = marker.title.split(" · ")[1] || "UBICACIÓN WRIXEL"; mapDetailCopy!.textContent = "Referencia visual de la caja dentro del juego."; if (mapDetailAction) mapDetailAction.textContent = marker.classList.contains("collected") ? "MARCADO COMO CONSEGUIDO" : "MARCAR COMO CONSEGUIDO"; };
