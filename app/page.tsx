...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const markerHtml = markers.map((marker) => `<button class="map-marker ${marker.kind}" data-map-kind="${marker.kind}" data-part-id="${marker.kind === "wrixel" ? marker.id : ""}" title="${marker.name} · ${marker.detail}" aria-label="${marker.name}" style="left:${marker.x}%;top:${marker.y}%"><span class="map-marker-dot"></span></button>`).join("");
    ...
    let zoom = 1;
    const setZoom = (next: number) => { zoom = Math.max(.75, Math.min(2.5, next)); if (canvas) canvas.style.width = `${zoom * 100}%`; };
