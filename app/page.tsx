...
const initialGustambitos: Gustambito[] = [
  ...
  { id: 4, name: "Tails", subtitle: "El compañero volador", rarity: "Épico", color: "#e98b42", image: spriteUrl("NarrowFlea_Monkey"), season: "GLITCH · Capítulo 7", variants: withLootHacker("NarrowFlea_Monkey", makeVariants("NarrowFlea_Monkey"), "NarrowFleaMonkey") },
...
const loadExportImage = (src: string) => new Promise<HTMLImageElement>((resolve, reject) => {
  ...
  image.onerror = () => {
    if (!src.startsWith("http")) return reject(new Error("No se pudo cargar la imagen"));
    image.onerror = reject;
    image.src = `${window.location.origin}/sprite-coming-soon.svg`;
  };
